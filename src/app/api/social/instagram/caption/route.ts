...
export async function POST(request: Request) {
  try {
    ...
    const prompt = `You are a senior Instagram content strategist writing for a professional brand.

Write a single, high-quality Instagram post caption for the following:
...
7. Output ONLY the caption text — no labels, no quotes, no preamble.`;
    ...
    console.error("[Instagram Caption Generation Error]:", err);
