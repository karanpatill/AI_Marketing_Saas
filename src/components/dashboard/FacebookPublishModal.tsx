...
export function FacebookPublishModal({
  ...
  const generateCaption = async () => {
    ...
    setError("");
    try {
      const res = await fetch("/api/social/facebook/caption", {
        method: "POST",
      ...
      <div className="relative w-full max-w-xl mx-4 bg-[#111111] border border-[#ffffff]/10 rounded-2xl shadow-[0_24px_80px_rgba(0,0,0,0.8)] overflow-hidden animate-fade-up">
        ...
        <div className="flex items-center justify-between px-6 py-4 border-b border-[#ffffff]/8">
          <div className="flex items-center gap-3">
            <div className="w-8 h-8 rounded-lg bg-[#1877F2] flex items-center justify-center shadow-lg">
              <Share2 className="w-4 h-4 text-white" />
            </div>
            <div>
              <h3 className="text-sm font-bold text-white tracking-tight">Publish to Facebook</h3>
              <p className="text-[11px] text-[#828282]">Review and publish your post to your Facebook Page</p>
            </div>
          </div>
          <button
            onClick={onClose}
