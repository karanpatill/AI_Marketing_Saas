...
export function renderSwissStyle(opt: TemplateOptions): string {
  const { title, content, category, brandName, website, primaryColor, secondaryColor, textColor, isLightBg, headlineFontStyle, bodyFontStyle, fontImportCss, aspectRatio, slideNum, totalSlides, bgImageUrl } = opt;
    ...
    <div class="relative w-full h-full bg-[${secondaryColor}] text-[${textColor}] overflow-hidden flex flex-col p-12" style="background-color: ${secondaryColor}; color: ${textColor}; aspect-ratio: ${aspectRatio.replace(':', '/')};">
      ...
      <div class="w-full flex justify-between items-center z-10 border-b ${borderCol} pb-4 mb-12">
        <div class="flex items-center gap-3">
          <svg width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M2 12h20M12 2v20"/></svg>
  ...
  return `
    ...
    <div class="relative w-full h-full overflow-hidden flex flex-col p-8" style="background-color: ${secondaryColor}; color: ${textColor}; aspect-ratio: ${aspectRatio.replace(':', '/')};">
      <div class="w-full h-full border p-8 flex flex-col relative" style="border-color: ${borderColor};">
        ...
        <div class="absolute top-0 left-1/2 -translate-x-1/2 -translate-y-1/2 bg-[${secondaryColor}] px-6" style="background-color: ${secondaryColor}">
...
export function renderGeneric(opt: TemplateOptions): string {
  ...
  return `
    ${fontImportCss ? `<style>${fontImportCss}</style>` : ''}
    <div class="relative w-full h-full p-10 flex flex-col justify-between overflow-hidden" style="background-color: ${secondaryColor}; color: ${textColor}; aspect-ratio: ${aspectRatio.replace(':', '/')};">
      ...
      <div class="w-full flex justify-between items-center shrink-0 border-b ${borderCol} pb-4 z-10 gap-3">
        <div class="flex items-center gap-3">
          ${logoUrl ? `<img src="${logoUrl}" alt="${brandName}" class="h-6 w-auto max-h-7 object-contain max-w-[110px]" />` : `<span class="font-bold tracking-wide">${brandName}</span>`}
        </div>
        <span class="text-[10px] uppercase font-bold tracking-widest px-3 py-1 rounded-full border ${isLightBg ? 'border-black/10 bg-black/5' : 'border-white/10 bg-white/5'}">
          ${category || 'INSIGHT'}
        </span>
      </div>
      <div class="flex-1 flex flex-col justify-center min-h-0 overflow-hidden py-8 gap-6 z-10 w-[90%]">
        <div class="w-12 h-1.5 rounded-full shrink-0" style="background-color: ${primaryColor};"></div>
        <h2 class="text-4xl md:text-5xl font-bold leading-tight" style="${headlineFontStyle}">${title}</h2>
        ${content ? `<p class="text-xl md:text-2xl leading-relaxed opacity-80" style="${bodyFontStyle}">${content}</p>` : ''}
      </div>
      <div class="w-full flex justify-between items-center shrink-0 border-t ${borderCol} pt-4 z-10">
        ${isCarousel 
          ? `<span class="text-[12px] font-bold tracking-widest opacity-60">${slideNum} / ${totalSlides}</span>`
          : `<span class="text-[10px] uppercase font-bold tracking-[0.2em] opacity-60">${brandName}</span>`
        }
        <span class="text-[10px] uppercase font-bold tracking-[0.2em] opacity-60">${website}</span>
      </div>
    </div>
  `;
}
