...
<div class="relative w-full h-full p-10 flex flex-col justify-between overflow-hidden select-none" style="background-color: ${bgBlack}; color: ${textCream}; aspect-ratio: ${aspectRatio.replace(':', '/')}; border: 1px solid rgba(225,224,204,0.15);">
  
  <div class="relative z-20 flex justify-between items-center w-full gap-3 pb-3 border-b border-[rgba(225,224,204,0.15)]">
    <div class="flex items-center gap-3">
      ...
      <span class="text-xs uppercase font-black tracking-widest px-3 py-1.5 rounded-full bg-[rgba(225,224,204,0.1)] border border-[rgba(225,224,204,0.2)] text-[${accentCream}]">
        ${s.category || 'INSIGHT'}
      </span>
    </div>
    <span class="text-xs font-mono font-bold tracking-widest" style="color: ${textMuted};">
      ${slideNum} / ${totalSlides}
    </span>
  </div>

  <div class="relative z-20 flex flex-col gap-5 w-full my-auto py-6">
    <div class="w-10 h-1.5 rounded-full mb-1 shrink-0" style="background-color: ${accentCream};"></div>
    <h2 class="${profile.headingClass} text-3xl md:text-4xl font-black tracking-tight leading-tight" style="${headlineFontStyle}">
      ${s.title || ''}
    </h2>
    ${s.content ? `
      <p class="${profile.bodyClass} text-lg leading-relaxed font-light" style="${bodyFontStyle}; color: ${textMuted};">
        ${s.content}
      </p>
    ` : ''}
  </div>

  <div class="relative z-20 flex justify-between items-center w-full pt-4 border-t border-[rgba(225,224,204,0.15)]">
    <span class="text-xs uppercase font-black tracking-[0.25em]" style="color: ${accentCream};">${brandName.toUpperCase()}</span>
    <span class="text-xs uppercase font-bold tracking-widest" style="color: ${textMuted};">SWIPE FOR MORE →</span>
  </div>
